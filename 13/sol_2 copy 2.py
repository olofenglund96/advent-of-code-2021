  ...
  for i, row in enumerate(grid.T):
    for j, val in enumerate(row):
      ...
      if val:
        ...
        add = f"{Style.DIM} {Style.RESET_ALL}"
...
def fold_grid(grid, axis, index):
  ...
  if ngrid.shape[0] % 2 == 0:
    extra = np.zeros([1, ngrid.shape[1]], dtype=bool)
    print(extra.shape)
    print(ngrid[index+1:].shape)
    print(ngrid[:index])
    grid_up, grid_down = ngrid[:index], np.concatenate((ngrid[index+1:], extra))
  else:
