...
def print_grid(grid, queue=None, path=[]):
  dash = "------------"
  printstr = f"{dash}\n"
  printstr += f"{dash}\n"
  qpoints = []
...
def compute_weight(grid, point):
  dx = abs(point[0] - grid.shape[0]+1)
  dy = abs(point[1] - grid.shape[1]+1)
  dist = dx + dy
...
def bfs(grid, explored, root):
  ...
  while len(q) > 0:
    ...
    if (i, j) == (grid.shape[0] - 1, grid.shape[1] - 1):
      return compute_path(path_dict, (i, j))
...
def main(lines):
  ...
  path = bfs(grid, explored, (0,0))

  print_grid(grid, queue=None, path=path)
  tot_sum = 0
  pathstr = ""
  for p in path:
    val = grid[p]
